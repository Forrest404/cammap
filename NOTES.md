...
## Where things are
    ...
    pages/              every other page - about, blog, rights, data,
                        press, account, report, moderate, leaderboard -
                        and boroughs/, the thirty-three borough pages
                        and their index, which are generated by
                        tools/build_boroughs.py and never edited by
                        hand ("The borough pages" below).
...
There was no way to ask "what is in Croydon" except to type it into the
list filter on the map, which finds cameras whose *name* says Croydon
and misses Thornton Heath and Selhurst Park. Thirty-four pages now
answer it: one per borough, and an index.
`tools/build_boroughs.py` writes all of them, and the borough block of
`sitemap.xml`, from `data/cameras.csv`.

**Generated, and checked.** Nothing on these pages is written by hand.
`stamp.py` regenerates all thirty-five files in memory on every run and
fails naming the file and the first differing line, exactly as it does
for `points.js`, `seed.sql` and the GeoJSON - so a hand edit to a
borough page cannot survive a commit, and neither can a record that
changed without the pages being rebuilt. It is not the build step this
project refuses, for `build_points.py`'s reason: the browser is served
what is committed, nothing runs for the site to work, and CI only
reads.
...
stylesheets and the nav), the footer, the tail (the `</div>` and every
`<script>` tag), and everything in the head that is not about which
page this is. Only eight tags are substituted: title, description,
canonical, `og:url`, `og:title`, `og:description`, `twitter:title`,
`twitter:description`.

That is the whole reason a CSP is not typed in the generator. Typed, it
would be a fifth copy for `stamp.py` to police and a human to get
wrong; copied, these pages carry whatever the site carries by
construction. When a page in `pages/` gains a script tag - the service
worker's - the borough pages gain it by being regenerated, and until
they are, `stamp.py` says so. **So: anything that changes the head, the
nav or the footer on every page is followed by
...
One thing `deepen()` cannot copy is `class="current"`, and it is taken
off. It marks which nav link is the page you are on, and on the
template that is Rights; copied as it stood, all thirty-four pages lit
Rights in the accent and told a visitor they were somewhere they were
not. It would have passed every check, because the nav comparison
strips the attribute as a per-page difference - which is right, and is
why this had to be caught by looking at the page.

**What a page says.** The borough's name; how many cameras the record
holds there, and how many of the pins mark the surrounding area rather
than a spot; a count per kind; every site, each linked into the map;
the busiest spot; the documents the borough's entries rest on; and the
date the record was generated from. Every number is counted at
generation time and none is typed, for the reason About and the count
line under the map give: a number in prose goes stale without a sound.

The count per kind is a list - `11  LFR van site` - rather than a
sentence, and that is a decision. A sentence needs a plural, and these
labels have none that can be derived: "LFR van site" takes an s,
...
There is no colour on these pages, deliberately. A swatch per kind is
the obvious thing, and the colours are in `CAMERA_TYPES` and are set as
inline styles by the scripts that draw the legend. These pages run no
JavaScript of their own - they are prose and read in full with scripts
off - so a swatch would mean a second copy of five colours in
`style.css`, which is exactly the copy the top of `shared.js` records
having deleted once already. The kind is said in words, which paper and
a screen reader wanted anyway.

**A site links to itself on the map** at
`../../index.html#camera=<url-encoded seed key>`. `map.js` resolves it
against the published record with no database at all, which is the
state somebody reading a static page about their borough is most likely
to be in; a numeric id would be shorter and would need the database.
Followed with the harness: the Croydon page's "North End, Croydon" link
opens the map at zoom 17 on 51.3785, -0.1025 with that camera's popup
open - the van site with 21 deployments, not the fixed camera at the
same coordinates - and Legacy switched on for the visit, because the
link asked and the visitor did not.

**A borough with no camera still has a page.** "No camera in the
record" is an answer to "what is in Bexley" and a 404 is not, and the
page says why it is not the same as "there are none": the sources name
no site there, and a shop's whole disclosure is the sign on its door.
All 33 have at least one camera today, from Barnet's and Sutton's one
to Westminster's 25, so that path was proved by moving Sutton's single
camera out in a scratch copy and reading the page it produced.

**The share card is the site's own,** `img/share.png`, inherited from
the template like everything else in the head. A per-borough card would
be a per-borough photograph of the map, and the only thing that makes
one is the screenshot harness "Sharing the site" describes - a real
browser driven by hand, which is not a tool in this repository and
cannot become one without the toolchain this project exists without.
The generator's docstring says where a per-borough image would go if a
harness is ever committed. The door is left open, not walked through.

**Byte-stable, which is what makes the check possible.** The boroughs
in `data/boroughs.txt`'s order, the sites in the record's own canonical
order, and no timestamp anywhere except `GENERATED` in the generator,
which is typed - the day the record was last looked at. It is
deliberately not read from `git log`: the check regenerates these pages
on a CI runner, in a shallow clone and in a tarball with no `.git` at
all, and "the last commit that touched the CSV" is none of those
reliably. It is the sitemap's `lastmod` for all 34 and the date each
page states, and it is updated when the pages are regenerated, the way
`RECORD_SOURCES` is when the record is refreshed.

**The sitemap.** The 34 addresses are a block between two comment
markers; everything outside them is the hand-written six and is never
touched. 43 URLs, parsed and counted.

**Not in the nav,** for `data.html` and `press.html`'s reason: the nav
has seven links and already wraps on a phone. The index is reached from
every borough page, and each borough page from the index. One line
would put it in front of everybody - the About page or the data page -
and that is an edit to a file this wave did not own.

**How it was checked.** `stamp.py --check` over 45 pages: one CSP, one
nav, one footer, and the generated check passing on all 35 files. A
hand-edited count, a hand-edited sitemap line, a record changed without
rebuilding, a blank borough and a borough emptied of cameras were each
tried in a scratch copy and each named the right file and line.
`stamp.py` with no flag re-stamps the template and all 34 pages
together and the check still passes, which is the order the merge will
run in. Three pages loaded at 1400 and 390 - Westminster's 25, the City
of London's two, and the index's 33 - with no horizontal scroll at
either width and the console carrying only the known
`frame-ancestors`-in-`<meta>` line. `sitemap.xml` parsed with
`xml.etree`: 43 URLs, no duplicates. `node tools/check.js`: 89 checks,
including that every camera's borough is one of the thirty-three and
that no two boroughs make the same page address.
