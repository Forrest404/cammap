...
404.html            what Pages serves for any missing address, at any
                    ...
                    forty-five today: eleven written by hand and the
                    thirty-four under pages/boroughs/ that are generated.
pages/boroughs/     the 33 borough pages and their index. GENERATED by
                    tools/build_boroughs.py from data/cameras.csv and
                    pages/rights.html; never edited by hand.
data/boroughs.txt   the 33 areas, the only copy of the list.
tools/boroughs.py   fills the record's borough column from Nominatim, by
                    hand, never by CI.
tools/build_boroughs.py  writes the borough pages, their index and the
                    sitemap's borough block.
...
- `worker-src 'self' blob:` — **both halves required, for different
  things.** `blob:` is MapLibre's tile workers, and without it the map does
  not draw at all. `'self'` is the service worker: this directive governs a
  service worker's own script as well, so `register("sw.js")` is refused
  outright without it — and the refusal names `worker-src`, not `script-src`,
  which is the confusing part. Take either token away and one of the two
...
- **A borough page is never edited by hand.** `stamp.py` regenerates all 35
  files in memory on every run and fails naming the page and the line.
...
- **The city.** `CITY` in `frontend/shared.js` is the name, the bounds, the
  centre and the opening zoom; `LONDON_BOUNDS` and `LONDON_CENTRE` are
  aliases of two of its fields, not copies — `check.js` asserts identity,
  because only that tells an alias from a copy. SQL's one copy is
  `public.in_city()`, which the three `*_in_london` constraints and
  `pending_near` all call; `stamp.py` holds the two languages together and
  refuses any other `lat/lon between` test in `schema.sql`. A second city is
  a second object and a switch, not a search and replace.
- **The borough column** lives in `data/cameras.csv` and
  `data/cameras.geojson` only — never in `points.js`, never in the database.
  A blank one builds and fails `check.js`. Run `tools/boroughs.py` when a
  camera is added, and when a moderator's Move is copied back into the CSV
  (clear the cell first: a moved camera may have crossed a boundary).
- **Anything that changes the head, nav or footer on every page is followed
  by `python3 tools/build_boroughs.py`.** The 34 borough pages copy all three
  from `pages/rights.html` at generation time, and `stamp.py` fails until
  they are rebuilt. The Content-Security-Policy gaining `worker-src 'self'`
  is exactly this case; it caught it.
...
CSP says `form-action 'none'` — and, until migration 012 is applied, one
404 on `cameras_public`, after which the documented fallback to the table
succeeds), the camera
