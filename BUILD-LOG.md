...
| KEEP-6 | 6 | city | verified | 672aea4 | *migration 013, unapplied*; `CITY` object with aliases; `public.in_city()` is SQL's one copy, called by the three constraints and `pending_near` | `CITY` object; stamp.py checks the SQL constraints against it |
| DATA-4 | 6 | city | verified | ef7eec0, dc566f4 | borough looked up once from Nominatim into the record; 33 pages and an index generated from the CSV and `rights.html`, checked byte-for-byte by `stamp.py` | borough pages generated by a tool, after KEEP-6 |
| KEEP-4 | 6 | offline | verified | 61b3b67 | `sw.js` + `frontend/offline.js`; shell precached under two hashes, ~300 tiles kept, Supabase and Nominatim never; update needs one reload, offered; CSP gained `worker-src 'self'` | `sw.js`, tested update path |
| KEEP-5 | 6 | freshness | verified | a022322 | cache drawn at once whatever its age, revalidated above a 30 s floor, redrawn on a row-signature difference; map, filters, popup, scroll, focus and hash all preserved | stale-while-revalidate without losing view state |
...
All 58 items of the brief are verified. The table holds 60 rows because DATA-1 and DATA-7
each occupy two: the schema half in Wave 1 and the surfacing half in Wave 5.
| blocked | 0 |
| dropped | 0 |
...
**Wave 6 summary (verified 2026-09-10).** Landed: a service worker precaching the shell and
the last ~300 tiles, with an update that reaches an old client through one offered reload
and never a forced one; the camera cache drawn at once and revalidated behind it, redrawing
without losing the map, the filters, the popup, the scroll or the focus; one `CITY` object
with `LONDON_BOUNDS` and `LONDON_CENTRE` as aliases of its fields, and `public.in_city()` as
SQL's single copy called by all three constraints and `pending_near` (migration 013,
**unapplied**); a borough looked up once per camera from Nominatim into the record, and 33
borough pages plus an index generated from it and checked byte-for-byte. Changed shape:
`worker-src` gained `'self'`, because that directive governs a service worker's script and
registration was refused without it; the worker registers locally only under `?offline`, so
edit-and-reload still shows the file on disk; `CAMERAS_TTL` became a 30-second floor.
Blocked: nothing.

**Final verification (2026-09-10).** All four met, and the whole tree checked rather than
asserted: the six invariants measured (no build step, from a pristine clone served with no
tool run; nothing estimated, with the scrubber filtering by recorded period and a null
source saying nothing; no email, name or IP column in any table and no function `anon` or
`authenticated` may call that takes a username or an email; the brightness rule measured at
119 for the halo, 103 for the badge and 92 for the Near me ring against a ceiling of 134;
all six `moderate_*` functions re-checking `is_moderator()` before any work, exercised live
as an ordinary user; and 128,340 comment words against 20,047 at the start, with no file
having lost its header); all 45 pages loaded at two widths with no overflow and no console
error beyond the known noise; and the whole migration path proved — the schema at `8bfad60`
plus 001 through 013 in order plus the seed dumps identical to the current schema and seed,
182 rows either way. **One real defect, the orchestrator's:** `404.html` had been moved into
`pages/` by a stray `git add -A` two commits earlier, which would have handed a rotted link
GitHub's own error page and silently undone REACH-4. Nothing caught it because the page set
is found by glob and no check had an opinion about where that one file sits. Fixed, and
`stamp.py` now checks it. Two documentation defects and five stale statements corrected in
the same commit. Totals: 58 of 58 verified.
