...
| DATA-1 (UI) | 5 | record | in-flight | | popup source line; null says nothing |
| DATA-7 (UI) | 5 | record | in-flight | | approximate drawn differently, legend entry, brightness rule |
| DATA-8 | 5 | record | in-flight | | published-record notice on failure/timeout |
| DATA-2 | 5 | record | in-flight | | `cameras.csv`, `cameras.geojson` from the generator |
| MAP-4 | 5 | record | in-flight | | period scrubber 2020–2026, filters, never predicts |
| DATA-6 | 5 | record | in-flight | | read API page with a working `curl` |
| REACH-6 | 5 | record | in-flight | | `pages/press.html` |
